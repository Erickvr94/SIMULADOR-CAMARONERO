...
// Tamaño BASE en píxeles al zoom de referencia (18). Se escala hacia abajo al
// alejar el mapa para que los aireadores no se amontonen visualmente y se
// pueda distinguir cuántos hay por piscina; y un poco hacia arriba al acercar.
const ANCHO_BASE = 30;
const ZOOM_REFERENCIA = 18;
const ANCHO_MIN = 8;
const ANCHO_MAX = 34;

function anchoParaZoom(zoom: number): number {
  const factor = Math.pow(1.3, zoom - ZOOM_REFERENCIA);
  return Math.round(Math.min(ANCHO_MAX, Math.max(ANCHO_MIN, ANCHO_BASE * factor)));
}

export function aireadorSvgIcon(estado: EstadoEquipo, seleccionado = false, zoom = ZOOM_REFERENCIA) {
  const clase = claseEstado[estado];
  const ancho = anchoParaZoom(zoom);
