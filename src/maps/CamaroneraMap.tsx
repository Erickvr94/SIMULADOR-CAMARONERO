...
/** Captura clics del mapa y los enruta según el modo activo, y trackea el zoom actual */
function CapturaClics() {
  ...
  const setZoom = useCamaroneraStore((s) => s.setZoom);
...
function PiscinaCapa({ piscina }: { piscina: Piscina }) {
  ...
  const zoom = useCamaroneraStore((s) => s.zoom);
    ...
    <Fragment>
      ...
      {piscina.aireadores.map((a) => (
        <Marker
          ...
          icon={aireadorSvgIcon(a.estado, seleccionada, zoom)}
